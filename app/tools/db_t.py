...
class TestPostgresConnection(ConnectionInterface):
    ...
    async def set_type_codec(self, *args, **kwargs):
        return await self._wrapped_connection.set_type_codec(
            *args , **kwargs
        )
        ...
        self.drop_query = """
           DO $$ DECLARE
            ...
            BEGIN
                ...
                -- Drop only custom enum types in the 'public' schema
                FOR obj IN (
                    SELECT typname AS name
                    FROM pg_type
                    WHERE typtype = 'e'  -- Only enums
                      AND typnamespace = 'public'::regnamespace  -- Only in the public schema
                      AND NOT EXISTS (
                          -- Exclude enums that are part of the system catalog
                          SELECT 1
                          FROM pg_catalog.pg_namespace ns
                          WHERE ns.oid = pg_type.typnamespace
                            AND ns.nspname = 'pg_catalog'
                      )
                ) LOOP
                    EXECUTE 'DROP TYPE IF EXISTS ' || quote_ident(obj.name) || ' CASCADE';
                END LOOP;
            END;
            $$;
        """
...
test_postgres_manager = TestPostgresManager('testsql.sql')
